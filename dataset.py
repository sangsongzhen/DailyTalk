...
class Dataset(Dataset):
        ...
        if self.history_type != "none":
            ...
            if self.history_type == "Guo":
                ...
                for i, h_basename in enumerate(history_basenames):
                    ...
                    # ⭐️ 新加：只加载当前说话人历史音频 embedding
                    if h_speaker == speaker:
                        h_audio_emb_path = os.path.join(
                            self.preprocessed_path,
                            "audio_emb",
                            "{}-audio_emb-{}.npy".format(h_speaker, h_basename),
                        )
                        history_audio_len += 1
                    ...
                    #     )
                    #     self.pad_history(
                    #         self.max_history_len-history_audio_len,
                    #         history_audio_emb=history_audio_emb # ⭐️ 新加
                    #     )
                # 强制统一长度到 max_history_len（必须在 for 循环结束后！）
                pad_len_text = self.max_history_len - len(history_text_emb)
                pad_len_audio = self.max_history_len - len(history_audio_emb)
                # pad_len = max(pad_len_text, pad_len_audio)
                ...
                # if turn == 0:
                if turn == 0 and len(history_text_emb) == 0 and len(history_audio_emb) == 0:
                    self.pad_history(
                ...
                # print("type(history_text_emb):", type(history_text_emb))
                # print("type(history_audio_emb):", type(history_audio_emb))

                # if len(history_text_emb) > 0 and len(history_audio_emb) > 0:
                #     print("type(history_text_emb[0]):", type(history_text_emb[0]))
                #     print("type(history_audio_emb[0]):", type(history_audio_emb[0]))

                #     print("shape(history_text_emb[0]):", np.array(history_text_emb[0]).shape)
                #     print("shape(history_audio_emb[0]):", np.array(history_audio_emb[0]).shape)

                # print("len(history_text_emb):", len(history_text_emb))
                # print("len(history_audio_emb):", len(history_audio_emb))
                # print("============================")
    ...
    def reprocess(self, data, idxs):
        ...
        if self.history_type != "none":
            if self.history_type == "Guo":
                text_embs = np.array([data[idx]["history"]["text_emb"] for idx in idxs])
                history_lens = np.array([data[idx]["history"]["history_len"] for idx in idxs])
        ...
        #         history_audio_embs = [data[idx]["history"]["history_audio_emb"] for idx in idxs]  # ✅ 新增
        #         # for i, x in enumerate(history_audio_embs):
        #         #     try:
        #         #         print(f"{i}: type={type(x)}, shape={(x.shape if hasattr(x, 'shape') else 'N/A')}, length={len(x) if hasattr(x, '__len__') else 'N/A'}")
        ...
        #         history_audio_embs = np.array(history_audio_embs)  # ✅ 新增
        #         # print(history_text_embs.shape)
        #         # # print(history_audio_embs.shape)
        #         # for i, x in enumerate(history_text_embs):
        #         #     print('history_text_embs \n')
        #         #     print(f"sample {i}: type={type(x)}, shape={x.shape if hasattr(x, 'shape') else 'N/A'}, length={len(x) if hasattr(x, '__len__') else 'N/A'}")
        #         # for i, x in enumerate(history_audio_embs):
        #         #     print('history_audio_embs \n')
        #         #     print(f"sample {i}: type={type(x)}, shape={x.shape if hasattr(x, 'shape') else 'N/A'}, length={len(x) if hasattr(x, '__len__') else 'N/A'}")
...
class TextDataset(Dataset):
    ...
    def collate_fn(self, data):
        ...
        if self.history_type != "none":
            if self.history_type == "Guo":
                text_embs = np.array([d[6]["text_emb"] for d in data])
                history_lens = np.array([d[6]["history_len"] for d in data])
