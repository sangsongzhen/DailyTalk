...
def train(rank, args, configs, batch_size):
        ...
        for batchs in loader:
            ...
            for batch in batchs:
                ...
                with torch.amp.autocast(device_type='cuda', dtype=torch.float16, enabled=args.use_amp):
                    # Forward
                    # for i, b in enumerate(batch[2:]):
                    #     print(f"batch[{i+2}] type: {type(b)} shape: {getattr(b, 'shape', 'N/A')}")
                ...
                if rank == 0:
                    # load_time = train_start - load_start
                    # train_time = time.time() - train_start
                    # print(f"[Step {step}] 加载时间: {load_time:.3f}s, 训练时间: {train_time:.3f}s")
                    if step % log_step == 0:
...
if __name__ == "__main__":
    ...
    print(" ---> use_film:", model_config["film"]["use_film"])
    ...
    train(0, args, configs, batch_size)
